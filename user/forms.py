...
class LoginForm(forms.Form):
    ...
    def clean(self):
        ...
        user = auth.authenticate(username=username_or_email, password=password)#验证用户输入是否是username
        if user is None: #如果不是username
            if User.objects.filter(email=username_or_email).exists():#如果是email并且存在
                username = User.objects.get(email=username_or_email).username#根据emial取得用户名信息
                user = auth.authenticate(username=username, password=password)#认证用户和密码是否有效
                if not user is None:#如果认证通过
                    self.cleaned_data['user'] = user 
                    return self.cleaned_data # 返回数据
            raise forms.ValidationError('用户名或密码不正确')#用户输入既不是username也不是emial 抛出异常
        else:
            self.cleaned_data['user'] = user #交给后端代码
        return self.cleaned_data # 返回cleaned_data数据
